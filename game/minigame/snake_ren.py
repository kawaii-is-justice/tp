...
class SnakeMinigameCDD(renpy.Displayable):
   ...
   BG_DISP = Solid("#777")
   STAGESIZ = 20  # Actual field size is 16*16.
   STAGEPAD = 4

   def __init__(self, **kwargs):
      super(SnakeMinigameCDD, self).__init__(**kwargs)
      self.lw = SnakeMinigameCDD.STAGESIZ  # Logical width
      self.lh = SnakeMinigameCDD.STAGESIZ  # Logical height
      self.pad = SnakeMinigameCDD.STAGEPAD
      self.tsiz = SnakeMinigameCDD.TILE_SIZE
      ...
      self.char = SnakeMinigameCDD.CHAR_RIGHT
      self.bg = SnakeMinigameCDD.BG_DISP
   ...
   def init_game(self):
      ...
      self.btem_pos = (center_x - 2, center_y - 2) # btem = bad item

   def end_game(self):
      self.game_ended = True
      self.char = SnakeMinigameCDD.CHAR_GAMEEND
   ...
   def event(self, e, x, y, st):
      ...
      if e.key == pygame.K_UP:
         ...
         self.direction = (0,-1)
         self.char = SnakeMinigameCDD.CHAR_UP
      elif e.key == pygame.K_DOWN:
         if self.direction == (0,-1): return
         self.direction = (0,1)
         self.char = SnakeMinigameCDD.CHAR_DOWN
      elif e.key == pygame.K_LEFT:
         if self.direction == (1,0): return
         self.direction = (-1,0)
         self.char = SnakeMinigameCDD.CHAR_LEFT
