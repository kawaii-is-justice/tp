label loop_0:
    ...
    "이후, 정신없는 자기소개 시간과 수업 시간이 지나가고 기대하던 점심 시간이 왔다."
    "선생님과 반 친구들이 나를 신기한 눈으로 쳐다보긴 했지만, 그럭저럭 잘 넘긴 것 같다."
    "이 학교의 급식은{cps=3}... {/cps}굉장히 맛있다고 한다. {w}게다가 점심 메뉴도 선택할 수 있다니, 기대가 된다."
    ...
    "누리는 내가 고른 메뉴를 따라 먹겠다고 했다. {w}첫날부터 밥 같이 먹을 사람이 생기다니... 참 다행이다."

    scene bg school classroom midday with Fade(1.0, 1.0, 1.0)
    show nuri smile at center, doup with dissolve
    ...
    hide nuri with dissolve
    scene bg school yard with Dissolve(1.0)
    ...
    "{cps=10}차는 땅에 곤두박질친 내 앞에 멈춰 섰다.{/cps}{w=1.0}{nw}"
