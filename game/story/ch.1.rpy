label loop_0:
    ...
    "이 학교의 급식은{cps=3}... {/cps}굉장히 맛있다고 한다..{p}게다가 점심 메뉴도 선택 가능하다니, 기대가 된다."
    ...
    "고양이를 구하기엔 충분한 시간이었지만,{p}사람이 빠져나오기엔 부족했다."

    play sound car_accident

    "{cps=10}누군가 소리치며 달려오는 게 들렸다.{p}아마... 누리겠지.{/cps}{w=1.0}{nw}"
    "{cps=10}차는 땅에 곤두박질 쳐진 내 앞에 멈춰섰다.{/cps}{w=1.0}{nw}"
    "{cps=10}아까 그 고양이는 나 따위는 안중에도 없다는 듯 도망갔다...{/cps}{w=1.0}{nw}"
