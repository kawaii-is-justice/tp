# Character definitions
# refer to https://www.renpy.org/doc/html/dialogue.html#the-character-store
define character.p = Character("[prtname]", color="#AAAAAA")  # protagonist
define character.n = Character("누리", color="#FFAEC9")
define character.h = Character("하늘", color="#99D9EA")

define character.p_noname = Character("주인공", color="#AAAAAA")
define character.who = Character("???", color="#777777")
define character.syswarn = Character("시스템 경고", color="#FF0000")

define character.pe_teacher = Character("체육 선생님", color="#777777")
define character.school_nurse = Character("보건 선생님", color="#777777")